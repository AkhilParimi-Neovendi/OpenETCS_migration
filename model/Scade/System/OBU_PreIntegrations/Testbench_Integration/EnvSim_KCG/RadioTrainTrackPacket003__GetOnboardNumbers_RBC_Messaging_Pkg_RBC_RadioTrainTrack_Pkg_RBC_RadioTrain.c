...
void RadioTrainTrackPacket003__GetOnboardNumbers_init_RBC_Messaging_Pkg_RBC_RadioTrainTrack_Pkg_RBC_Radio(
  outC_RadioTrainTrackPacket003__GetOnboardNumbers_RBC_Messaging_Pkg_RBC_RadioTrainTrack_Pkg_RBC_Radio *outC)
{
  static kcg_size idx;
  ...
  for (idx1 = 0; idx1 < 1; idx1++) {
    ...
    for (idx = 0; idx < 15; idx++) {
      outC->_L2[idx1].telephoneNumber[idx] = kcg_lit_int64(0);
    }
  }
  ...
  for (idx3 = 0; idx3 < 1; idx3++) {
    ...
    for (idx2 = 0; idx2 < 15; idx2++) {
      outC->_L1.aNID_RADIO[idx3].telephoneNumber[idx2] = kcg_lit_int64(0);
    }
  }
  for (idx5 = 0; idx5 < 1; idx5++) {
    outC->outOnboardPhoneNumbers[idx5].valid = kcg_true;
    for (idx4 = 0; idx4 < 15; idx4++) {
      outC->outOnboardPhoneNumbers[idx5].telephoneNumber[idx4] = kcg_lit_int64(0);
    }
  }
}
...
void RadioTrainTrackPacket003__GetOnboardNumbers_reset_RBC_Messaging_Pkg_RBC_RadioTrainTrack_Pkg_RBC_Radi(
  outC_RadioTrainTrackPacket003__GetOnboardNumbers_RBC_Messaging_Pkg_RBC_RadioTrainTrack_Pkg_RBC_Radio *outC)
{
}
