...
void addGradient_init_SDMModelPkg(outC_addGradient_SDMModelPkg *outC)
{
  ...
  for (idx3 = 0; idx3 < 14; idx3++) {
    for (idx2 = 0; idx2 < 100; idx2++) {
      outC->_L32[idx3][idx2] = kcg_lit_float32(0.0);
    }
  }
  for (idx4 = 0; idx4 < 100; idx4++) {
    outC->_L31[idx4].position = kcg_lit_float32(0.0);
    outC->_L31[idx4].gradient_acceleration = kcg_lit_float32(0.0);
    outC->_L31[idx4].valid = kcg_true;
  }
  for (idx6 = 0; idx6 < 14; idx6++) {
    for (idx5 = 0; idx5 < 100; idx5++) {
      outC->_L30[idx6][idx5].position = kcg_lit_float32(0.0);
      outC->_L30[idx6][idx5].gradient_acceleration = kcg_lit_float32(0.0);
      outC->_L30[idx6][idx5].valid = kcg_true;
    }
  }
  for (idx7 = 0; idx7 < 100; idx7++) {
    outC->_L29.distance_definition[idx7] = kcg_lit_float32(0.0);
  }
  for (idx8 = 0; idx8 < 14; idx8++) {
    outC->_L29.speed_definition[idx8] = kcg_lit_float32(0.0);
  }
  for (idx10 = 0; idx10 < 14; idx10++) {
    for (idx9 = 0; idx9 < 100; idx9++) {
      outC->_L29.data[idx10][idx9] = kcg_lit_float32(0.0);
    }
  }
  for (idx11 = 0; idx11 < 100; idx11++) {
    outC->_L28[idx11] = kcg_lit_float32(0.0);
  }
  for (idx12 = 0; idx12 < 100; idx12++) {
    outC->_L27.distance_definition[idx12] = kcg_lit_float32(0.0);
  }
  for (idx13 = 0; idx13 < 14; idx13++) {
    outC->_L27.speed_definition[idx13] = kcg_lit_float32(0.0);
  }
  for (idx15 = 0; idx15 < 14; idx15++) {
    for (idx14 = 0; idx14 < 100; idx14++) {
      outC->_L27.data[idx15][idx14] = kcg_lit_float32(0.0);
    }
  }
  for (idx17 = 0; idx17 < 14; idx17++) {
    for (idx16 = 0; idx16 < 100; idx16++) {
      outC->_L26[idx17][idx16] = kcg_lit_float32(0.0);
    }
  }
  outC->_L20 = kcg_lit_float32(0.0);
  outC->_L17 = kcg_lit_float32(0.0);
  for (idx18 = 0; idx18 < 14; idx18++) {
    outC->_L13[idx18] = kcg_lit_float32(0.0);
  }
  for (idx19 = 0; idx19 < 100; idx19++) {
    outC->_L12.distance_definition[idx19] = kcg_lit_float32(0.0);
  }
  for (idx20 = 0; idx20 < 14; idx20++) {
    outC->_L12.speed_definition[idx20] = kcg_lit_float32(0.0);
  }
  for (idx22 = 0; idx22 < 14; idx22++) {
    for (idx21 = 0; idx21 < 100; idx21++) {
      outC->_L12.data[idx22][idx21] = kcg_lit_float32(0.0);
    }
  }
  for (idx23 = 0; idx23 < 100; idx23++) {
    outC->_L2[idx23].position = kcg_lit_float32(0.0);
    outC->_L2[idx23].gradient_acceleration = kcg_lit_float32(0.0);
    outC->_L2[idx23].valid = kcg_true;
  }
  for (idx24 = 0; idx24 < 100; idx24++) {
    outC->aSafe.distance_definition[idx24] = kcg_lit_float32(0.0);
  }
  for (idx25 = 0; idx25 < 14; idx25++) {
    outC->aSafe.speed_definition[idx25] = kcg_lit_float32(0.0);
  }
  for (idx27 = 0; idx27 < 14; idx27++) {
    for (idx26 = 0; idx26 < 100; idx26++) {
      outC->aSafe.data[idx27][idx26] = kcg_lit_float32(0.0);
    }
  }
