  ...
  outC_makeEmptyParabolaCurve_CalcBrakingCurves_types *outC)
{
  ...
  for (idx2 = 0; idx2 < 114; idx2++) {
    outC->_L13.distances[idx2] = kcg_lit_float64(0.0);
  }
  for (idx3 = 0; idx3 < 114; idx3++) {
    outC->_L13.speeds[idx3] = kcg_lit_float64(0.0);
  }
  for (idx4 = 0; idx4 < 114; idx4++) {
    outC->_L13.accelerations[idx4] = kcg_lit_float64(0.0);
  }
  for (idx5 = 0; idx5 < 114; idx5++) {
    outC->_L13.valid[idx5] = kcg_true;
  }
  for (idx6 = 0; idx6 < 114; idx6++) {
    outC->Curve.distances[idx6] = kcg_lit_float64(0.0);
  }
  for (idx7 = 0; idx7 < 114; idx7++) {
    outC->Curve.speeds[idx7] = kcg_lit_float64(0.0);
  }
  for (idx8 = 0; idx8 < 114; idx8++) {
    outC->Curve.accelerations[idx8] = kcg_lit_float64(0.0);
  }
