...
void TrainData__Set_NID_ENGINE_RBC_Session_Pkg(
  ...
  /* in_NID_ENGINE/ */
  NID_ENGINE in_NID_ENGINE,
  outC_TrainData__Set_NID_ENGINE_RBC_Session_Pkg *outC)
{
  outC->_L3 = in_NID_ENGINE;
  kcg_copy_TrainData_T(&outC->_L1, inTrainData);
  kcg_copy_TrainData_T(&outC->_L2, &outC->_L1);
  if (kcg_true) {
    outC->_L2.nid_engine = outC->_L3;
  }
  kcg_copy_TrainData_T(&outC->outTrainData, &outC->_L2);
}

#ifndef KCG_USER_DEFINED_INIT
void TrainData__Set_NID_ENGINE_init_RBC_Session_Pkg(
  outC_TrainData__Set_NID_ENGINE_RBC_Session_Pkg *outC)
{
  static kcg_size idx;
  ...
  outC->_L2.m_NumberOfOnboardPhoneNumbers = kcg_lit_int32(0);
  for (idx1 = 0; idx1 < 1; idx1++) {
    outC->_L2.m_OnboardPhoneNumbers[idx1].valid = kcg_true;
    for (idx = 0; idx < 15; idx++) {
      outC->_L2.m_OnboardPhoneNumbers[idx1].telephoneNumber[idx] = kcg_lit_int32(0);
    }
  }
  outC->_L2.t_train_ref = kcg_lit_int32(0);
  ...
  outC->_L2.n_axle = kcg_lit_int32(0);
  for (idx2 = 0; idx2 < 4; idx2++) {
    outC->_L2.m_TractionIdentities[idx2].m_voltage =
      M_VOLTAGE_Line_not_fitted_with_any_traction_system;
    outC->_L2.m_TractionIdentities[idx2].nid_ctraction = kcg_lit_int32(0);
  }
  for (idx3 = 0; idx3 < 5; idx3++) {
    outC->_L2.m_NationalSystemIdentities[idx3] = kcg_lit_int32(0);
  }
...
void TrainData__Set_NID_ENGINE_reset_RBC_Session_Pkg(
  outC_TrainData__Set_NID_ENGINE_RBC_Session_Pkg *outC)
{
}
