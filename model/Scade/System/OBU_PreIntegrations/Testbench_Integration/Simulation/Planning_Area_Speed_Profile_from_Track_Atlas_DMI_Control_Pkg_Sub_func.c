  ...
  outC_Planning_Area_Speed_Profile_from_Track_Atlas_DMI_Control_Pkg_Sub_func *outC)
{
  ...
  for (idx = /* _L6= */(kcg_size) outC->_L6; idx < 32; idx++) {
    ...
    outC->_L4[idx] = kcg_lit_float64(0.0);
  }
#endif /* KCG_MAPW_CPY */

  noname = outC->_L6;
  kcg_copy_array_float64_32(&outC->MRSP_to_DMI, &outC->_L4);
  kcg_copy_array_float64_32(&outC->Loc_Abs_to_DMI, &outC->_L2);
}
  ...
  outC_Planning_Area_Speed_Profile_from_Track_Atlas_DMI_Control_Pkg_Sub_func *outC)
{
  ...
  for (idx2 = 0; idx2 < 32; idx2++) {
    outC->_L2[idx2] = kcg_lit_float64(0.0);
  }
  for (idx3 = 0; idx3 < 32; idx3++) {
    outC->_L1[idx3].valid = kcg_true;
    outC->_L1[idx3].Loc_Abs = kcg_lit_int64(0);
    outC->_L1[idx3].Loc_LRBG = kcg_lit_int64(0);
    outC->_L1[idx3].MRS = kcg_lit_int64(0);
  }
  for (idx4 = 0; idx4 < 32; idx4++) {
    outC->MRSP_to_DMI[idx4] = kcg_lit_float64(0.0);
  }
  for (idx5 = 0; idx5 < 32; idx5++) {
    outC->Loc_Abs_to_DMI[idx5] = kcg_lit_float64(0.0);
  }
  for (idx = 0; idx < 32; idx++) {
    /* _L6=(DMI_Control_Pkg::Sub_func::Planning_Area_Speed_Profile_List#1)/ */
