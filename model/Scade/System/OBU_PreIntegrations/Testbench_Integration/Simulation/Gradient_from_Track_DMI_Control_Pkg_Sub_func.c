  ...
  outC_Gradient_from_Track_DMI_Control_Pkg_Sub_func *outC)
{
  ...
  if (outC->_L29) {
    ...
    for (idx = 0; idx < 32; idx++) {
      ...
      outC->_L24 = /* _L24= */(kcg_int32) (idx + 1);
      /* _L24= */
  ...
  for (idx = /* _L24= */(kcg_size) outC->_L24; idx < 32; idx++) {
    ...
    outC->_L28[idx] = kcg_lit_float32(0.0);
    outC->_L25[idx] = kcg_lit_float32(0.0);
    outC->_L26[idx] = kcg_lit_float32(0.0);
  }
#endif /* KCG_MAPW_CPY */

  kcg_copy_array_bool_32(&outC->Gradient_Visible_Flag, &outC->_L31);
  noname = outC->_L24;
  kcg_copy_array_float32_32(&outC->Gradient_Value, &outC->_L26);
  kcg_copy_array_float32_32(&outC->Gradient_Begin, &outC->_L28);
  kcg_copy_array_float32_32(&outC->Gradient_End, &outC->_L25);
}
  ...
  outC_Gradient_from_Track_DMI_Control_Pkg_Sub_func *outC)
{
  ...
  for (idx5 = 0; idx5 < 32; idx5++) {
    ...
    outC->_L12[idx5].begin_section = kcg_lit_int32(0);
    outC->_L12[idx5].end_section = kcg_lit_int32(0);
    outC->_L12[idx5].gradient = kcg_lit_int32(0);
  }
  for (idx6 = 0; idx6 < 32; idx6++) {
    outC->Gradient_Visible_Flag[idx6] = kcg_true;
  }
  for (idx7 = 0; idx7 < 32; idx7++) {
    outC->Gradient_Value[idx7] = kcg_lit_float32(0.0);
  }
  for (idx8 = 0; idx8 < 32; idx8++) {
    outC->Gradient_Begin[idx8] = kcg_lit_float32(0.0);
  }
  for (idx9 = 0; idx9 < 32; idx9++) {
    outC->Gradient_End[idx9] = kcg_lit_float32(0.0);
  }
  for (idx = 0; idx < 32; idx++) {
    /* _L24=(DMI_Control_Pkg::Sub_func::Gradient_Profile_LIst_Operator#1)/ */
