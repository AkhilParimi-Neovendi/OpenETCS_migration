...
void RBC_close_init_RadioLib(outC_RBC_close_RadioLib *outC)
{
  ...
  for (idx4 = 0; idx4 < 30; idx4++) {
    outC->Packets_out.PacketHeaders[idx4].nid_packet = kcg_lit_int32(0);
    outC->Packets_out.PacketHeaders[idx4].q_dir = Q_DIR_Reverse;
    outC->Packets_out.PacketHeaders[idx4].valid = kcg_true;
    outC->Packets_out.PacketHeaders[idx4].startAddress = kcg_lit_int32(0);
    outC->Packets_out.PacketHeaders[idx4].endAddress = kcg_lit_int32(0);
  }
  for (idx5 = 0; idx5 < 500; idx5++) {
    outC->Packets_out.PacketData[idx5] = kcg_lit_int32(0);
  }
