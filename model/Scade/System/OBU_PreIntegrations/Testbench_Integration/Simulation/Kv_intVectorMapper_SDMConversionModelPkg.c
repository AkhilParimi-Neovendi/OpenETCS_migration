  ...
  outC_Kv_intVectorMapper_SDMConversionModelPkg *outC)
{
  ...
  static array_int64_2 acc;
  static kcg_size idx3;
  static array_int64_2 noname;
  ...
  kcg_copy_array_int64_2(&outC->_L12, &outC->_L17);
  /* _L12= */
  for (idx3 = 0; idx3 < 13; idx3++) {
    kcg_copy_array_int64_2(&acc, &outC->_L12);
  ...
  kcg_copy_array_int64_2(&noname, &outC->_L12);
  ...
  outC_Kv_intVectorMapper_SDMConversionModelPkg *outC)
{
  ...
  for (idx1 = 0; idx1 < 13; idx1++) {
    outC->_L26.a[idx1] = kcg_lit_int64(0);
  }
  for (idx2 = 0; idx2 < 13; idx2++) {
    outC->_L26.v[idx2] = kcg_lit_int64(0);
  }
  for (idx3 = 0; idx3 < 13; idx3++) {
  ...
  for (idx4 = 0; idx4 < 2; idx4++) {
    outC->_L17[idx4] = kcg_lit_int64(0);
  }
  for (idx7 = 0; idx7 < 13; idx7++) {
    for (idx5 = 0; idx5 < 6; idx5++) {
      outC->_L16[idx7].a[idx5] = kcg_lit_int64(0);
    }
    for (idx6 = 0; idx6 < 6; idx6++) {
      outC->_L16[idx7].v[idx6] = kcg_lit_int64(0);
    }
  }
  for (idx10 = 0; idx10 < 13; idx10++) {
    for (idx8 = 0; idx8 < 7; idx8++) {
      outC->_L14[idx10].m[idx8] = kcg_lit_int64(0);
    }
    for (idx9 = 0; idx9 < 7; idx9++) {
      outC->_L14[idx10].v[idx9] = kcg_lit_int64(0);
    }
  }
  for (idx11 = 0; idx11 < 2; idx11++) {
    outC->_L12[idx11] = kcg_lit_int64(0);
  }
  for (idx12 = 0; idx12 < 13; idx12++) {
    outC->_L13[idx12] = kcg_lit_int64(0);
  }
  for (idx13 = 0; idx13 < 7; idx13++) {
    outC->_L2.m[idx13] = kcg_lit_int64(0);
  }
  for (idx14 = 0; idx14 < 7; idx14++) {
    outC->_L2.v[idx14] = kcg_lit_int64(0);
  }
  for (idx15 = 0; idx15 < 6; idx15++) {
    outC->_L1.a[idx15] = kcg_lit_int64(0);
  }
  for (idx16 = 0; idx16 < 6; idx16++) {
    outC->_L1.v[idx16] = kcg_lit_int64(0);
  }
  for (idx17 = 0; idx17 < 13; idx17++) {
    outC->A_brake_safe.a[idx17] = kcg_lit_int64(0);
  }
  for (idx18 = 0; idx18 < 13; idx18++) {
    outC->A_brake_safe.v[idx18] = kcg_lit_int64(0);
  }
