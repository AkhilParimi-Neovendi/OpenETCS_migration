...
void RBC_init_init_RadioLib(outC_RBC_init_RadioLib *outC)
{
  ...
  for (idx4 = 0; idx4 < 30; idx4++) {
    outC->R_data_out.packets.PacketHeaders[idx4].nid_packet = kcg_lit_int32(0);
    outC->R_data_out.packets.PacketHeaders[idx4].q_dir = Q_DIR_Reverse;
    outC->R_data_out.packets.PacketHeaders[idx4].valid = kcg_true;
    outC->R_data_out.packets.PacketHeaders[idx4].startAddress = kcg_lit_int32(0);
    outC->R_data_out.packets.PacketHeaders[idx4].endAddress = kcg_lit_int32(0);
  }
  for (idx5 = 0; idx5 < 500; idx5++) {
    outC->R_data_out.packets.PacketData[idx5] = kcg_lit_int32(0);
  }
