  ...
  outC_SortByDistances_CalcBrakingCurves_Pkg_internalOperators *outC)
{
  ...
  if (outC->_L22) {
    ...
    for (idx4 = 0; idx4 < 114; idx4++) {
      ...
      outC->_L21 = /* _L21= */(kcg_int64) (idx4 + 1);
      /* _L21= */
  ...
  outC_SortByDistances_CalcBrakingCurves_Pkg_internalOperators *outC)
{
  ...
  outC->_L21 = kcg_lit_int64(0);
  for (idx2 = 0; idx2 < 114; idx2++) {
    outC->_L19.distances[idx2] = kcg_lit_float64(0.0);
  }
  for (idx3 = 0; idx3 < 114; idx3++) {
    outC->_L19.speeds[idx3] = kcg_lit_float64(0.0);
  }
  for (idx4 = 0; idx4 < 114; idx4++) {
    outC->_L19.accelerations[idx4] = kcg_lit_float64(0.0);
  }
  for (idx5 = 0; idx5 < 114; idx5++) {
    outC->_L19.valid[idx5] = kcg_true;
  }
  for (idx6 = 0; idx6 < 114; idx6++) {
    outC->_L18.distances[idx6] = kcg_lit_float64(0.0);
  }
  for (idx7 = 0; idx7 < 114; idx7++) {
    outC->_L18.speeds[idx7] = kcg_lit_float64(0.0);
  }
  for (idx8 = 0; idx8 < 114; idx8++) {
    outC->_L18.accelerations[idx8] = kcg_lit_float64(0.0);
  }
  for (idx9 = 0; idx9 < 114; idx9++) {
    outC->_L18.valid[idx9] = kcg_true;
  }
  for (idx10 = 0; idx10 < 114; idx10++) {
    outC->_L9[idx10] = kcg_lit_float64(0.0);
  }
  for (idx11 = 0; idx11 < 114; idx11++) {
    outC->_L8[idx11] = kcg_lit_float64(0.0);
  }
  ...
  for (idx14 = 0; idx14 < 114; idx14++) {
    outC->_L4[idx14] = kcg_lit_float64(0.0);
  }
  for (idx15 = 0; idx15 < 114; idx15++) {
    outC->_L3[idx15] = kcg_lit_float64(0.0);
  }
  for (idx16 = 0; idx16 < 114; idx16++) {
    outC->_L2[idx16] = kcg_lit_float64(0.0);
  }
  for (idx17 = 0; idx17 < 114; idx17++) {
    outC->_L1.distances[idx17] = kcg_lit_float64(0.0);
  }
  for (idx18 = 0; idx18 < 114; idx18++) {
    outC->_L1.speeds[idx18] = kcg_lit_float64(0.0);
  }
  for (idx19 = 0; idx19 < 114; idx19++) {
    outC->_L1.accelerations[idx19] = kcg_lit_float64(0.0);
  }
  for (idx20 = 0; idx20 < 114; idx20++) {
    outC->_L1.valid[idx20] = kcg_true;
  }
  for (idx21 = 0; idx21 < 114; idx21++) {
    outC->sortedBC.distances[idx21] = kcg_lit_float64(0.0);
  }
  for (idx22 = 0; idx22 < 114; idx22++) {
    outC->sortedBC.speeds[idx22] = kcg_lit_float64(0.0);
  }
  for (idx23 = 0; idx23 < 114; idx23++) {
    outC->sortedBC.accelerations[idx23] = kcg_lit_float64(0.0);
  }
