  ...
  outC_CalculateEBDFootForMRSPorLOA_CalcBrakingCurves_Pkg_internalOperators *outC)
{
  ...
  for (idx1 = 0; idx1 < 100; idx1++) {
    outC->_L17.distance_definition[idx1] = kcg_lit_float64(0.0);
  }
  for (idx2 = 0; idx2 < 14; idx2++) {
    outC->_L17.speed_definition[idx2] = kcg_lit_float64(0.0);
  }
  for (idx4 = 0; idx4 < 14; idx4++) {
    for (idx3 = 0; idx3 < 100; idx3++) {
      outC->_L17.data[idx4][idx3] = kcg_lit_float64(0.0);
    }
  }
  outC->_L16.targetType = EoA_TargetManagement_types;
  outC->_L16.distance = kcg_lit_float64(0.0);
  outC->_L16.speed = kcg_lit_float64(0.0);
  outC->_L14.distance = kcg_lit_float64(0.0);
  outC->_L14.speed = kcg_lit_float64(0.0);
  outC->_L14.distanceIndex = kcg_lit_int64(0);
  outC->_L14.speedIndex = kcg_lit_int64(0);
  for (idx5 = 0; idx5 < 114; idx5++) {
    outC->_L14.BC.distances[idx5] = kcg_lit_float64(0.0);
  }
  for (idx6 = 0; idx6 < 114; idx6++) {
    outC->_L14.BC.speeds[idx6] = kcg_lit_float64(0.0);
  }
  for (idx7 = 0; idx7 < 114; idx7++) {
    outC->_L14.BC.accelerations[idx7] = kcg_lit_float64(0.0);
  }
  for (idx8 = 0; idx8 < 114; idx8++) {
    outC->_L14.BC.valid[idx8] = kcg_true;
  }
  for (idx9 = 0; idx9 < 114; idx9++) {
    outC->_L12.distances[idx9] = kcg_lit_float64(0.0);
  }
  for (idx10 = 0; idx10 < 114; idx10++) {
    outC->_L12.speeds[idx10] = kcg_lit_float64(0.0);
  }
  for (idx11 = 0; idx11 < 114; idx11++) {
    outC->_L12.accelerations[idx11] = kcg_lit_float64(0.0);
  }
  ...
  for (idx13 = 0; idx13 < 114; idx13++) {
    outC->_L9.distances[idx13] = kcg_lit_float64(0.0);
  }
  for (idx14 = 0; idx14 < 114; idx14++) {
    outC->_L9.speeds[idx14] = kcg_lit_float64(0.0);
  }
  for (idx15 = 0; idx15 < 114; idx15++) {
    outC->_L9.accelerations[idx15] = kcg_lit_float64(0.0);
  }
  for (idx16 = 0; idx16 < 114; idx16++) {
    outC->_L9.valid[idx16] = kcg_true;
  }
  for (idx17 = 0; idx17 < 113; idx17++) {
    outC->_L8[idx17] = kcg_lit_float64(0.0);
  }
  for (idx22 = 0; idx22 < 113; idx22++) {
    for (idx18 = 0; idx18 < 100; idx18++) {
      outC->_L7[idx22].distance_definition[idx18] = kcg_lit_float64(0.0);
    }
    for (idx19 = 0; idx19 < 14; idx19++) {
      outC->_L7[idx22].speed_definition[idx19] = kcg_lit_float64(0.0);
    }
    for (idx21 = 0; idx21 < 14; idx21++) {
      for (idx20 = 0; idx20 < 100; idx20++) {
        outC->_L7[idx22].data[idx21][idx20] = kcg_lit_float64(0.0);
      }
    }
  }
  ...
  outC->_L5.distanceIndex = kcg_lit_int64(0);
  outC->_L5.speedIndex = kcg_lit_int64(0);
  for (idx23 = 0; idx23 < 114; idx23++) {
    outC->_L5.BC.distances[idx23] = kcg_lit_float64(0.0);
  }
  for (idx24 = 0; idx24 < 114; idx24++) {
    outC->_L5.BC.speeds[idx24] = kcg_lit_float64(0.0);
  }
  for (idx25 = 0; idx25 < 114; idx25++) {
    outC->_L5.BC.accelerations[idx25] = kcg_lit_float64(0.0);
  }
  ...
  for (idx27 = 0; idx27 < 100; idx27++) {
    outC->_L3.distance_definition[idx27] = kcg_lit_float64(0.0);
  }
  for (idx28 = 0; idx28 < 14; idx28++) {
    outC->_L3.speed_definition[idx28] = kcg_lit_float64(0.0);
  }
  for (idx30 = 0; idx30 < 14; idx30++) {
    for (idx29 = 0; idx29 < 100; idx29++) {
      outC->_L3.data[idx30][idx29] = kcg_lit_float64(0.0);
    }
  }
  outC->EBDfoot = kcg_lit_float64(0.0);
  /* _L10=(CalcBrakingCurves_types::getLocationOnCurve#1)/ */
