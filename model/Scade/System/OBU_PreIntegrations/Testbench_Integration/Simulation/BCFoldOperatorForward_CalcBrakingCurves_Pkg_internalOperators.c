  ...
  outC_BCFoldOperatorForward_CalcBrakingCurves_Pkg_internalOperators *outC)
{
  ...
  outC->_L58 = Index;
  /* _L61=(CalcBrakingCurves_Pkg::internalOperators::setArc#1)/ */
  ...
  kcg_copy_ParabolaCurve_T_CalcBrakingCurves_types(&outC->_L42.BC, &outC->_L61);
  kcg_copy_BCAccumulator_type_CalcBrakingCurves_Pkg(&outC->BC, &outC->_L42);
  outC->_L33 = LOA_MRSP_Target_speed;
  outC->_L32 = outC->_L33 <= outC->_L2;
  outC->exitCond = outC->_L32;
}

#ifndef KCG_USER_DEFINED_INIT
void BCFoldOperatorForward_init_CalcBrakingCurves_Pkg_internalOperators(
  outC_BCFoldOperatorForward_CalcBrakingCurves_Pkg_internalOperators *outC)
{
  static kcg_size idx;
  ...
  for (idx = 0; idx < 114; idx++) {
    outC->_L61.distances[idx] = kcg_lit_float64(0.0);
  }
  for (idx1 = 0; idx1 < 114; idx1++) {
    outC->_L61.speeds[idx1] = kcg_lit_float64(0.0);
  }
  for (idx2 = 0; idx2 < 114; idx2++) {
    outC->_L61.accelerations[idx2] = kcg_lit_float64(0.0);
  }
  for (idx3 = 0; idx3 < 114; idx3++) {
    outC->_L61.valid[idx3] = kcg_true;
  }
  outC->_L59 = kcg_true;
  outC->_L58 = kcg_lit_int64(0);
  outC->_L42.distance = kcg_lit_float64(0.0);
  outC->_L42.speed = kcg_lit_float64(0.0);
  outC->_L42.distanceIndex = kcg_lit_int64(0);
  outC->_L42.speedIndex = kcg_lit_int64(0);
  for (idx4 = 0; idx4 < 114; idx4++) {
    outC->_L42.BC.distances[idx4] = kcg_lit_float64(0.0);
  }
  for (idx5 = 0; idx5 < 114; idx5++) {
    outC->_L42.BC.speeds[idx5] = kcg_lit_float64(0.0);
  }
  for (idx6 = 0; idx6 < 114; idx6++) {
    outC->_L42.BC.accelerations[idx6] = kcg_lit_float64(0.0);
  }
  for (idx7 = 0; idx7 < 114; idx7++) {
    outC->_L42.BC.valid[idx7] = kcg_true;
  }
  for (idx8 = 0; idx8 < 114; idx8++) {
    outC->_L37.distances[idx8] = kcg_lit_float64(0.0);
  }
  for (idx9 = 0; idx9 < 114; idx9++) {
    outC->_L37.speeds[idx9] = kcg_lit_float64(0.0);
  }
  for (idx10 = 0; idx10 < 114; idx10++) {
    outC->_L37.accelerations[idx10] = kcg_lit_float64(0.0);
  }
  ...
  outC->_L36.distance = kcg_lit_float64(0.0);
  outC->_L36.speed = kcg_lit_float64(0.0);
  outC->_L36.distanceIndex = kcg_lit_int64(0);
  outC->_L36.speedIndex = kcg_lit_int64(0);
  for (idx12 = 0; idx12 < 114; idx12++) {
    outC->_L36.BC.distances[idx12] = kcg_lit_float64(0.0);
  }
  for (idx13 = 0; idx13 < 114; idx13++) {
    outC->_L36.BC.speeds[idx13] = kcg_lit_float64(0.0);
  }
  for (idx14 = 0; idx14 < 114; idx14++) {
    outC->_L36.BC.accelerations[idx14] = kcg_lit_float64(0.0);
  }
  ...
  for (idx16 = 0; idx16 < 100; idx16++) {
    outC->_L6.distance_definition[idx16] = kcg_lit_float64(0.0);
  }
  for (idx17 = 0; idx17 < 14; idx17++) {
    outC->_L6.speed_definition[idx17] = kcg_lit_float64(0.0);
  }
  for (idx19 = 0; idx19 < 14; idx19++) {
    for (idx18 = 0; idx18 < 100; idx18++) {
      outC->_L6.data[idx19][idx18] = kcg_lit_float64(0.0);
    }
  }
  ...
  outC->BC.distance = kcg_lit_float64(0.0);
  outC->BC.speed = kcg_lit_float64(0.0);
  outC->BC.distanceIndex = kcg_lit_int64(0);
  outC->BC.speedIndex = kcg_lit_int64(0);
  for (idx20 = 0; idx20 < 114; idx20++) {
    outC->BC.BC.distances[idx20] = kcg_lit_float64(0.0);
  }
  for (idx21 = 0; idx21 < 114; idx21++) {
    outC->BC.BC.speeds[idx21] = kcg_lit_float64(0.0);
  }
  for (idx22 = 0; idx22 < 114; idx22++) {
    outC->BC.BC.accelerations[idx22] = kcg_lit_float64(0.0);
  }
...
void BCFoldOperatorForward_reset_CalcBrakingCurves_Pkg_internalOperators(
  outC_BCFoldOperatorForward_CalcBrakingCurves_Pkg_internalOperators *outC)
{
  /* _L61=(CalcBrakingCurves_Pkg::internalOperators::setArc#1)/ */
