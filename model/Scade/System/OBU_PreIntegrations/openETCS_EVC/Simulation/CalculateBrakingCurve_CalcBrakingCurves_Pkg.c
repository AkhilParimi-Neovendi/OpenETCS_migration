  ...
  outC_CalculateBrakingCurve_CalcBrakingCurves_Pkg *outC)
{
  kcg_size idx;
  kcg_size idx1;
  BCAccumulator_type_CalcBrakingCurves_Pkg acc;
  kcg_size idx2;
  BCAccumulator_type_CalcBrakingCurves_Pkg op_call;
  kcg_int32 noname;
  ...
  outC_CalculateBrakingCurve_CalcBrakingCurves_Pkg *outC)
{
  ...
  for (idx1 = 0; idx1 < 100; idx1++) {
    outC->_L137.distance_definition[idx1] = kcg_lit_float32(0.0);
  }
  for (idx2 = 0; idx2 < 14; idx2++) {
    outC->_L137.speed_definition[idx2] = kcg_lit_float32(0.0);
  }
  for (idx4 = 0; idx4 < 14; idx4++) {
    for (idx3 = 0; idx3 < 100; idx3++) {
      outC->_L137.data[idx4][idx3] = kcg_lit_float32(0.0);
    }
  }
  outC->_L136.targetType = EoA_TargetManagement_types;
  outC->_L136.distance = kcg_lit_float32(0.0);
  outC->_L136.speed = kcg_lit_float32(0.0);
  outC->_L134.distance = kcg_lit_float32(0.0);
  outC->_L134.speed = kcg_lit_float32(0.0);
  outC->_L134.distanceIndex = kcg_lit_int32(0);
  outC->_L134.speedIndex = kcg_lit_int32(0);
  for (idx5 = 0; idx5 < 114; idx5++) {
    outC->_L134.BC.distances[idx5] = kcg_lit_float32(0.0);
  }
  for (idx6 = 0; idx6 < 114; idx6++) {
    outC->_L134.BC.speeds[idx6] = kcg_lit_float32(0.0);
  }
  for (idx7 = 0; idx7 < 114; idx7++) {
    outC->_L134.BC.accelerations[idx7] = kcg_lit_float32(0.0);
  }
  for (idx8 = 0; idx8 < 114; idx8++) {
    outC->_L134.BC.valid[idx8] = kcg_true;
  }
  for (idx9 = 0; idx9 < 114; idx9++) {
    outC->_L125.distances[idx9] = kcg_lit_float32(0.0);
  }
  for (idx10 = 0; idx10 < 114; idx10++) {
    outC->_L125.speeds[idx10] = kcg_lit_float32(0.0);
  }
  for (idx11 = 0; idx11 < 114; idx11++) {
    outC->_L125.accelerations[idx11] = kcg_lit_float32(0.0);
  }
  for (idx12 = 0; idx12 < 114; idx12++) {
    outC->_L125.valid[idx12] = kcg_true;
  }
  for (idx13 = 0; idx13 < 114; idx13++) {
    outC->_L114.distances[idx13] = kcg_lit_float32(0.0);
  }
  for (idx14 = 0; idx14 < 114; idx14++) {
    outC->_L114.speeds[idx14] = kcg_lit_float32(0.0);
  }
  for (idx15 = 0; idx15 < 114; idx15++) {
    outC->_L114.accelerations[idx15] = kcg_lit_float32(0.0);
  }
  for (idx16 = 0; idx16 < 114; idx16++) {
    outC->_L114.valid[idx16] = kcg_true;
  }
  outC->_L97.distance = kcg_lit_float32(0.0);
  outC->_L97.speed = kcg_lit_float32(0.0);
  outC->_L97.distanceIndex = kcg_lit_int32(0);
  outC->_L97.speedIndex = kcg_lit_int32(0);
  for (idx17 = 0; idx17 < 114; idx17++) {
    outC->_L97.BC.distances[idx17] = kcg_lit_float32(0.0);
  }
  for (idx18 = 0; idx18 < 114; idx18++) {
    outC->_L97.BC.speeds[idx18] = kcg_lit_float32(0.0);
  }
  for (idx19 = 0; idx19 < 114; idx19++) {
    outC->_L97.BC.accelerations[idx19] = kcg_lit_float32(0.0);
  }
  for (idx20 = 0; idx20 < 114; idx20++) {
    outC->_L97.BC.valid[idx20] = kcg_true;
  }
  outC->_L19.distance = kcg_lit_float32(0.0);
  outC->_L19.speed = kcg_lit_float32(0.0);
  outC->_L19.distanceIndex = kcg_lit_int32(0);
  outC->_L19.speedIndex = kcg_lit_int32(0);
  for (idx21 = 0; idx21 < 114; idx21++) {
    outC->_L19.BC.distances[idx21] = kcg_lit_float32(0.0);
  }
  for (idx22 = 0; idx22 < 114; idx22++) {
    outC->_L19.BC.speeds[idx22] = kcg_lit_float32(0.0);
  }
  for (idx23 = 0; idx23 < 114; idx23++) {
    outC->_L19.BC.accelerations[idx23] = kcg_lit_float32(0.0);
  }
  ...
  for (idx25 = 0; idx25 < 100; idx25++) {
    outC->_L13.distance_definition[idx25] = kcg_lit_float32(0.0);
  }
  for (idx26 = 0; idx26 < 14; idx26++) {
    outC->_L13.speed_definition[idx26] = kcg_lit_float32(0.0);
  }
  for (idx28 = 0; idx28 < 14; idx28++) {
    for (idx27 = 0; idx27 < 100; idx27++) {
      outC->_L13.data[idx28][idx27] = kcg_lit_float32(0.0);
    }
  }
  for (idx29 = 0; idx29 < 113; idx29++) {
    outC->_L12[idx29] = kcg_lit_float32(0.0);
  }
  for (idx34 = 0; idx34 < 113; idx34++) {
    for (idx30 = 0; idx30 < 100; idx30++) {
      outC->_L11[idx34].distance_definition[idx30] = kcg_lit_float32(0.0);
    }
    for (idx31 = 0; idx31 < 14; idx31++) {
      outC->_L11[idx34].speed_definition[idx31] = kcg_lit_float32(0.0);
    }
    for (idx33 = 0; idx33 < 14; idx33++) {
      for (idx32 = 0; idx32 < 100; idx32++) {
        outC->_L11[idx34].data[idx33][idx32] = kcg_lit_float32(0.0);
      }
    }
  }
  outC->_L9 = kcg_lit_int32(0);
  ...
  for (idx35 = 0; idx35 < 114; idx35++) {
    outC->BC.distances[idx35] = kcg_lit_float32(0.0);
  }
  for (idx36 = 0; idx36 < 114; idx36++) {
    outC->BC.speeds[idx36] = kcg_lit_float32(0.0);
  }
  for (idx37 = 0; idx37 < 114; idx37++) {
    outC->BC.accelerations[idx37] = kcg_lit_float32(0.0);
  }
