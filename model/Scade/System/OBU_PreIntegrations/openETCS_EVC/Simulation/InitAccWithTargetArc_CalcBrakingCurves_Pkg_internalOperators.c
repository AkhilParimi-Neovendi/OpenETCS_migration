  ...
  outC_InitAccWithTargetArc_CalcBrakingCurves_Pkg_internalOperators *outC)
{
  ...
  V_internal_real_Type_SDM_Types_Pkg _1_noname;
  ...
  outC_InitAccWithTargetArc_CalcBrakingCurves_Pkg_internalOperators *outC)
{
  ...
  outC->_L68.distance = kcg_lit_float32(0.0);
  outC->_L68.speed = kcg_lit_float32(0.0);
  outC->_L68.acceleration = kcg_lit_float32(0.0);
  for (idx = 0; idx < 114; idx++) {
    outC->_L64.distances[idx] = kcg_lit_float32(0.0);
  }
  for (idx1 = 0; idx1 < 114; idx1++) {
    outC->_L64.speeds[idx1] = kcg_lit_float32(0.0);
  }
  for (idx2 = 0; idx2 < 114; idx2++) {
    outC->_L64.accelerations[idx2] = kcg_lit_float32(0.0);
  }
  for (idx3 = 0; idx3 < 114; idx3++) {
    outC->_L64.valid[idx3] = kcg_true;
  }
  for (idx4 = 0; idx4 < 114; idx4++) {
    outC->_L63.distances[idx4] = kcg_lit_float32(0.0);
  }
  for (idx5 = 0; idx5 < 114; idx5++) {
    outC->_L63.speeds[idx5] = kcg_lit_float32(0.0);
  }
  for (idx6 = 0; idx6 < 114; idx6++) {
    outC->_L63.accelerations[idx6] = kcg_lit_float32(0.0);
  }
  ...
  outC->_L33.distance = kcg_lit_float32(0.0);
  outC->_L33.speed = kcg_lit_float32(0.0);
  ...
  for (idx9 = 0; idx9 < 14; idx9++) {
    for (idx8 = 0; idx8 < 100; idx8++) {
      outC->_L7[idx9][idx8] = kcg_lit_float32(0.0);
    }
  }
  for (idx10 = 0; idx10 < 14; idx10++) {
    outC->_L6[idx10] = kcg_lit_float32(0.0);
  }
  for (idx11 = 0; idx11 < 100; idx11++) {
    outC->_L5[idx11] = kcg_lit_float32(0.0);
  }
  outC->_L4.distance = kcg_lit_float32(0.0);
  outC->_L4.speed = kcg_lit_float32(0.0);
  outC->_L4.distanceIndex = kcg_lit_int32(0);
  outC->_L4.speedIndex = kcg_lit_int32(0);
  for (idx12 = 0; idx12 < 114; idx12++) {
    outC->_L4.BC.distances[idx12] = kcg_lit_float32(0.0);
  }
  for (idx13 = 0; idx13 < 114; idx13++) {
    outC->_L4.BC.speeds[idx13] = kcg_lit_float32(0.0);
  }
  for (idx14 = 0; idx14 < 114; idx14++) {
    outC->_L4.BC.accelerations[idx14] = kcg_lit_float32(0.0);
  }
  for (idx15 = 0; idx15 < 114; idx15++) {
    outC->_L4.BC.valid[idx15] = kcg_true;
  }
  for (idx16 = 0; idx16 < 100; idx16++) {
    outC->_L1.distance_definition[idx16] = kcg_lit_float32(0.0);
  }
  for (idx17 = 0; idx17 < 14; idx17++) {
    outC->_L1.speed_definition[idx17] = kcg_lit_float32(0.0);
  }
  for (idx19 = 0; idx19 < 14; idx19++) {
    for (idx18 = 0; idx18 < 100; idx18++) {
      outC->_L1.data[idx19][idx18] = kcg_lit_float32(0.0);
    }
  }
  outC->_L2.targetType = EoA_TargetManagement_types;
  outC->_L2.distance = kcg_lit_float32(0.0);
  outC->_L2.speed = kcg_lit_float32(0.0);
  outC->Output1.distance = kcg_lit_float32(0.0);
  outC->Output1.speed = kcg_lit_float32(0.0);
  outC->Output1.distanceIndex = kcg_lit_int32(0);
  outC->Output1.speedIndex = kcg_lit_int32(0);
  for (idx20 = 0; idx20 < 114; idx20++) {
    outC->Output1.BC.distances[idx20] = kcg_lit_float32(0.0);
  }
  for (idx21 = 0; idx21 < 114; idx21++) {
    outC->Output1.BC.speeds[idx21] = kcg_lit_float32(0.0);
  }
  for (idx22 = 0; idx22 < 114; idx22++) {
    outC->Output1.BC.accelerations[idx22] = kcg_lit_float32(0.0);
  }
