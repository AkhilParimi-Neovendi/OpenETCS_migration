...
{
  av_Map_t_SDMConversionModelPkg op_call;
  ...
  outC_brakePercentToABrake_SDMConversionModelPkg *outC)
{
  ...
  for (idx = 0; idx < 5; idx++) {
    outC->_L9.nationSystems[idx] = kcg_lit_int32(0);
  }
  outC->_L9.numberTractionSystems = kcg_lit_int32(0);
  for (idx1 = 0; idx1 < 4; idx1++) {
    outC->_L9.tractionSystem[idx1].m_voltage =
      M_VOLTAGE_Line_not_fitted_with_any_traction_system;
    outC->_L9.tractionSystem[idx1].nid_ctraction = kcg_lit_int32(0);
  }
  outC->_L10 = kcg_lit_int32(0);
  for (idx2 = 0; idx2 < 6; idx2++) {
    outC->_L8.emergency.a[idx2] = kcg_lit_int32(0);
  }
  for (idx3 = 0; idx3 < 6; idx3++) {
    outC->_L8.emergency.v[idx3] = kcg_lit_int32(0);
  }
  for (idx4 = 0; idx4 < 6; idx4++) {
    outC->_L8.service.a[idx4] = kcg_lit_int32(0);
  }
  for (idx5 = 0; idx5 < 6; idx5++) {
    outC->_L8.service.v[idx5] = kcg_lit_int32(0);
  }
  outC->_L5 = kcg_true;
  outC->_L4 = kcg_lit_int32(0);
  for (idx6 = 0; idx6 < 6; idx6++) {
    outC->_L3.a[idx6] = kcg_lit_int32(0);
  }
  for (idx7 = 0; idx7 < 6; idx7++) {
    outC->_L3.v[idx7] = kcg_lit_int32(0);
  }
  for (idx8 = 0; idx8 < 6; idx8++) {
    outC->_L1.a[idx8] = kcg_lit_int32(0);
  }
  for (idx9 = 0; idx9 < 6; idx9++) {
    outC->_L1.v[idx9] = kcg_lit_int32(0);
  }
  outC->every = kcg_true;
  for (idx10 = 0; idx10 < 6; idx10++) {
    outC->aBrake.emergency.a[idx10] = kcg_lit_int32(0);
  }
  for (idx11 = 0; idx11 < 6; idx11++) {
    outC->aBrake.emergency.v[idx11] = kcg_lit_int32(0);
  }
  for (idx12 = 0; idx12 < 6; idx12++) {
    outC->aBrake.service.a[idx12] = kcg_lit_int32(0);
  }
  for (idx13 = 0; idx13 < 6; idx13++) {
    outC->aBrake.service.v[idx13] = kcg_lit_int32(0);
  }
  /* _L1=(SDMConversionModelPkg::brakePercentLookUp#1)/ */
