...
void TrainData__SetOnboardPhon_RBC_Session_Pkg(
  /* inTrainData/ */
...
void TrainData__SetOnboardPhon_init_RBC_Session_Pkg(
  outC_TrainData__SetOnboardPhon_RBC_Session_Pkg *outC)
{
  kcg_size idx;
  kcg_size idx1;
  ...
  for (idx5 = 0; idx5 < 1; idx5++) {
    ...
    for (idx4 = 0; idx4 < 15; idx4++) {
      outC->_L2[idx5].telephoneNumber[idx4] = kcg_lit_int64(0);
    }
  }
  ...
  for (idx7 = 0; idx7 < 1; idx7++) {
    ...
    for (idx6 = 0; idx6 < 15; idx6++) {
      outC->_L1.m_OnboardPhoneNumbers[idx7].telephoneNumber[idx6] = kcg_lit_int64(0);
    }
  }
  ...
  for (idx8 = 0; idx8 < 4; idx8++) {
    outC->_L1.m_TractionIdentities[idx8].m_voltage =
      M_VOLTAGE_Line_not_fitted_with_any_traction_system;
    outC->_L1.m_TractionIdentities[idx8].nid_ctraction = kcg_lit_int64(0);
  }
  for (idx9 = 0; idx9 < 5; idx9++) {
    outC->_L1.m_NationalSystemIdentities[idx9] = kcg_lit_int64(0);
  }
  ...
  for (idx12 = 0; idx12 < 4; idx12++) {
    outC->outTrainData.m_TractionIdentities[idx12].m_voltage =
      M_VOLTAGE_Line_not_fitted_with_any_traction_system;
    outC->outTrainData.m_TractionIdentities[idx12].nid_ctraction = kcg_lit_int64(0);
  }
  for (idx13 = 0; idx13 < 5; idx13++) {
    outC->outTrainData.m_NationalSystemIdentities[idx13] = kcg_lit_int64(0);
  }
...
void TrainData__SetOnboardPhon_reset_RBC_Session_Pkg(
  outC_TrainData__SetOnboardPhon_RBC_Session_Pkg *outC)
{
}
...
/* $********** SCADE Suite KCG 32-bit 6.6.2 (build i4) **********
** TrainData__SetOnboardPhon_RBC_Session_Pkg.c
** Generation date: 2023-09-12T13:45:51
*************************************************************$ */
