...
void TrainData__SetTractionIde_RBC_Session_Pkg(
  /* inTrainData/ */
  TrainData_T *inTrainData,
  /* inTractionIdentity/ */
  aTractionIdentity_T_Packet_TrainTypes_Pkg *inTractionIdentity,
  outC_TrainData__SetTractionIde_RBC_Session_Pkg *outC)
{
...
void TrainData__SetTractionIde_init_RBC_Session_Pkg(
  outC_TrainData__SetTractionIde_RBC_Session_Pkg *outC)
{
  kcg_size idx;
  kcg_size idx1;
  ...
  for (idx4 = 0; idx4 < 4; idx4++) {
    outC->_L2[idx4].m_voltage = M_VOLTAGE_Line_not_fitted_with_any_traction_system;
    outC->_L2[idx4].nid_ctraction = kcg_lit_int64(0);
  }
  outC->_L1.nid_engine = kcg_lit_int64(0);
  outC->_L1.m_NumberOfOnboardPhoneNumbers = kcg_lit_int64(0);
  for (idx6 = 0; idx6 < 1; idx6++) {
    outC->_L1.m_OnboardPhoneNumbers[idx6].valid = kcg_true;
    for (idx5 = 0; idx5 < 15; idx5++) {
      outC->_L1.m_OnboardPhoneNumbers[idx6].telephoneNumber[idx5] = kcg_lit_int64(0);
    }
  }
  ...
  for (idx7 = 0; idx7 < 4; idx7++) {
    outC->_L1.m_TractionIdentities[idx7].m_voltage =
      M_VOLTAGE_Line_not_fitted_with_any_traction_system;
    outC->_L1.m_TractionIdentities[idx7].nid_ctraction = kcg_lit_int64(0);
  }
  for (idx8 = 0; idx8 < 5; idx8++) {
    outC->_L1.m_NationalSystemIdentities[idx8] = kcg_lit_int64(0);
  }
  ...
  for (idx10 = 0; idx10 < 1; idx10++) {
    outC->outTrainData.m_OnboardPhoneNumbers[idx10].valid = kcg_true;
    for (idx9 = 0; idx9 < 15; idx9++) {
      outC->outTrainData.m_OnboardPhoneNumbers[idx10].telephoneNumber[idx9] =
        kcg_lit_int64(0);
    }
  }
  ...
  for (idx11 = 0; idx11 < 4; idx11++) {
    outC->outTrainData.m_TractionIdentities[idx11].m_voltage =
      M_VOLTAGE_Line_not_fitted_with_any_traction_system;
    outC->outTrainData.m_TractionIdentities[idx11].nid_ctraction = kcg_lit_int64(0);
  }
  for (idx12 = 0; idx12 < 5; idx12++) {
    outC->outTrainData.m_NationalSystemIdentities[idx12] = kcg_lit_int64(0);
  }
...
void TrainData__SetTractionIde_reset_RBC_Session_Pkg(
  outC_TrainData__SetTractionIde_RBC_Session_Pkg *outC)
{
}
...
/* $********** SCADE Suite KCG 32-bit 6.6.2 (build i4) **********
** TrainData__SetTractionIde_RBC_Session_Pkg.c
** Generation date: 2023-09-12T13:45:51
*************************************************************$ */
