  ...
  outC_TrainData__SetTractionIde_RBC_Session_Pkg *outC)
{
  ...
  for (idx4 = 0; idx4 < 4; idx4++) {
    ...
    outC->_L2[idx4].nid_ctraction = kcg_lit_int32(0);
  }
  ...
  for (idx6 = 0; idx6 < 1; idx6++) {
    ...
    for (idx5 = 0; idx5 < 15; idx5++) {
      outC->_L1.m_OnboardPhoneNumbers[idx6].telephoneNumber[idx5] = kcg_lit_int32(0);
    }
  }
  ...
  for (idx7 = 0; idx7 < 4; idx7++) {
    outC->_L1.m_TractionIdentities[idx7].m_voltage =
      M_VOLTAGE_Line_not_fitted_with_any_traction_system;
    outC->_L1.m_TractionIdentities[idx7].nid_ctraction = kcg_lit_int32(0);
  }
  for (idx8 = 0; idx8 < 5; idx8++) {
    outC->_L1.m_NationalSystemIdentities[idx8] = kcg_lit_int32(0);
  }
  ...
  for (idx11 = 0; idx11 < 4; idx11++) {
    outC->outTrainData.m_TractionIdentities[idx11].m_voltage =
      M_VOLTAGE_Line_not_fitted_with_any_traction_system;
    outC->outTrainData.m_TractionIdentities[idx11].nid_ctraction = kcg_lit_int32(0);
  }
  for (idx12 = 0; idx12 < 5; idx12++) {
    outC->outTrainData.m_NationalSystemIdentities[idx12] = kcg_lit_int32(0);
  }
