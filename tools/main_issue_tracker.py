...
def main_issue_tracker(  ):
    ...
    [closed_count_TITLE,closed_WEEKS_NR,closed_count_URL,closed_count_LABELS]= get_closed_ISSUE(closed_ISSUE, not_closed_ISSUE, ISSUE_AXES)
    print "done Getting closed issues"

    print "Getting not closed issues"
    [not_count_TITLE,not_WEEKS_NR,not_count_URL,not_count_LABELS]= get_not_closed_ISSUE(closed_ISSUE, not_closed_ISSUE, ISSUE_AXES);
    print "done Getting not closed issues"

    print "Writing files for not closed issues"
    writting_file_not_closed_issues(not_count_TITLE,not_WEEKS_NR,not_count_URL,not_count_LABELS)
    print "Done Writing files for not closed issues"

    print "Writing files for closed issues"
    writting_file_closed_issues(closed_count_TITLE,closed_WEEKS_NR,closed_count_URL,closed_count_LABELS)
    print "Done Writing files not closed issues"




#working on modularization

def get_closed_ISSUE(closed_ISSUE, not_closed_ISSUE, ISSUE_AXES):
    ...
    for i in range(len(closed_ISSUE_MA[:,ISSUE_AXES['ISSUE_CLOSED']])):
        if closed_ISSUE_MA[i,ISSUE_AXES['ISSUE_CLOSED']] is not None:
            duration.append( current_TIME - closed_ISSUE_MA[i,ISSUE_AXES['ISSUE_CLOSED']])
            closed_ISSUE_URL.append(closed_ISSUE[i][ISSUE_AXES['ISSUE_URL']])
            closed_ISSUE_TITLE.append(closed_ISSUE[i][ISSUE_AXES['ISSUE_TITLE']])
            closed_ISSUE_LABELS.append(closed_ISSUE[i][ISSUE_AXES['ISSUE_LABELS']])
        else:
            print "is completed"
    ...
    for i in range(len(closed_ISSUE_MA[:,ISSUE_AXES['ISSUE_CLOSED']])):
        if duration[i].days < 8:
            ...
            count_URL[WEEKS_NR['ONE']].append(closed_ISSUE_URL[i])
            count_TITLE[WEEKS_NR['ONE']].append(closed_ISSUE_TITLE[i])
            count_LABELS[WEEKS_NR['ONE']].append(closed_ISSUE_LABELS[i])
        elif duration[i].days > 7 and duration[i].days < 15:
            count_WEEKS[WEEKS_NR['TWO']]=count_WEEKS[WEEKS_NR['TWO']] +1;
            count_URL[WEEKS_NR['TWO']].append(closed_ISSUE_URL[i])
            count_TITLE[WEEKS_NR['TWO']].append(closed_ISSUE_TITLE[i])
            count_LABELS[WEEKS_NR['TWO']].append(closed_ISSUE_LABELS[i])
        elif duration[i].days > 14 and duration[i].days < 22:
            count_WEEKS[WEEKS_NR['THREE']]=count_WEEKS[WEEKS_NR['THREE']] +1;
            count_URL[WEEKS_NR['THREE']].append(closed_ISSUE_URL[i])
            count_TITLE[WEEKS_NR['THREE']].append(closed_ISSUE_TITLE[i])
            count_LABELS[WEEKS_NR['THREE']].append(closed_ISSUE_LABELS[i])
        elif duration[i].days > 21 and duration[i].days < 29:
            count_WEEKS[WEEKS_NR['FOUR']]=count_WEEKS[WEEKS_NR['FOUR']] +1;
            count_URL[WEEKS_NR['FOUR']].append(closed_ISSUE_URL[i])
            count_TITLE[WEEKS_NR['FOUR']].append(closed_ISSUE_TITLE[i])
            count_LABELS[WEEKS_NR['FOUR']].append(closed_ISSUE_LABELS[i])
        elif duration[i].days > 28 and duration[i].days < 36:
            count_WEEKS[WEEKS_NR['FIVE']]=count_WEEKS[WEEKS_NR['FIVE']] +1;
            count_URL[WEEKS_NR['FIVE']].append(closed_ISSUE_URL[i])
            count_TITLE[WEEKS_NR['FIVE']].append(closed_ISSUE_TITLE[i])
            count_LABELS[WEEKS_NR['FIVE']].append(closed_ISSUE_LABELS[i])
        elif duration[i].days > 35 and duration[i].days < 43:
            count_WEEKS[WEEKS_NR['SIX']]=count_WEEKS[WEEKS_NR['SIX']] +1;
            count_URL[WEEKS_NR['SIX']].append(closed_ISSUE_URL[i])
            count_TITLE[WEEKS_NR['SIX']].append(closed_ISSUE_TITLE[i])
            count_LABELS[WEEKS_NR['SIX']].append(closed_ISSUE_LABELS[i])
        elif duration[i].days > 42 and duration[i].days < 50:
            count_WEEKS[WEEKS_NR['SEVEN']]=count_WEEKS[WEEKS_NR['SEVEN']] +1;
            count_URL[WEEKS_NR['SEVEN']].append(closed_ISSUE_URL[i])
            count_TITLE[WEEKS_NR['SEVEN']].append(closed_ISSUE_TITLE[i])
            count_LABELS[WEEKS_NR['SEVEN']].append(closed_ISSUE_LABELS[i])
        elif duration[i].days > 49:
            count_WEEKS[WEEKS_NR['GREATER_THEN']]=count_WEEKS[WEEKS_NR['GREATER_THEN']] +1;
            count_URL[WEEKS_NR['GREATER_THEN']].append(closed_ISSUE_URL[i])
            count_TITLE[WEEKS_NR['GREATER_THEN']].append(closed_ISSUE_TITLE[i])
            count_LABELS[WEEKS_NR['GREATER_THEN']].append(closed_ISSUE_LABELS[i])
    return [count_TITLE,WEEKS_NR,count_URL,count_LABELS]


#here a new function
def get_not_closed_ISSUE(closed_ISSUE, not_closed_ISSUE, ISSUE_AXES):
...
def writting_file_not_closed_issues(count_TITLE,WEEKS_NR,count_URL,count_LABELS):
...
def writting_file_closed_issues(count_TITLE,WEEKS_NR,count_URL,count_LABELS):

    #writing the required files
    f = open('Week_1_closed_issues.txt', 'w');
    ...
    f = open('Week_2_closed_issues.txt', 'w');
    ...
    f = open('Week_3_closed_issues.txt', 'w');
